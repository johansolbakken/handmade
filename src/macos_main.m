...
uint64_t mac_get_wall_clock(void) { return mach_absolute_time(); }
...
static void audio_callback(void *user_data, AudioQueueRef inAQ,
  ...
  // Build a game_sound_output_buffer structure to pass to our sound generation
  // function.
  game_sound_output_buffer sound_buffer = {0};
  sound_buffer.samples_per_second =
      48000; // or use a field from your state if stored there
...
void init_audio(void) {
  ...
  OSStatus status =
      AudioQueueNewOutput(&format,
                          ...
                          kCFRunLoopCommonModes, 0, &global_audio_state.queue);
  check_error(status, "AudioQueueNewOutput");
  if (status != noErr)
    return;

  // Allocate some buffers. The AudioQueue will call our callback when it needs
  // data. For example, allocate 3 buffers each holding ~0.1 second of audio.
  float seconds_per_buffer = 0.1f;
  int frames_per_buffer =
      (int)(sound_output.samples_per_second * seconds_per_buffer);
  ...
  for (int i = 0; i < 3; i++) {
    status =
        AudioQueueAllocateBuffer(global_audio_state.queue, bytes_per_buffer,
    ...
    audio_callback(&global_audio_state, global_audio_state.queue,
                   global_audio_state.buffers[i]);
  }
